...
## 候选故事 8:Memory 独家聚合事实 0%→40% — RAG KB 冗余却挡不住 Memory 不可替代价值(6.3 主线 1)

**6.3 实测**(2026-06-01,`ablation_6_3_report.md §1`):full vs -Mem0 在 50 strategy 上的画像锚定,三度量全显著且 c=0(full 从不输)。

**最硬的金子 = 度量 2(Mem0 独家聚合事实「85% 客群结构」)**:full 40% vs -Mem0 **严格 0%**(χ²=20.0)。

**故事张力(诚实驱动)**:6.3 判画像锚定时浮现一个**反直觉的冗余**——RAG 知识库是商家专属撰写的,价格带 ¥100-300 / 学生职场客群 / 主播分工**本就在 KB 文档里**,所以 -Mem0(RAG 开)也能锚定基础画像(价格带 full 26 vs -Mem0 25,几乎冗余)。CC 一度据此推「主线 1 可能 nil」。**但全量数据否决了过早判断**:KB 不含的「85% 客群结构」聚合事实,只有 Mem0 提供——full 40% vs -Mem0 0%。

**为何 0% 是金子**:不是「加了 Memory 好一点」,是「关掉 Memory 就**完全消失**的独家事实」。RAG 和电商常识都给不了的画像精确数字,只有时序记忆能提供。**这是 Memory 项目梦寐以求的消融结果**——不可替代性的硬证据。

**产品映射(客户端落地)**:本 demo 的基础画像冗余是「给了专属 KB」的数据特性;**真实客户端(抖音商家端)没有为每个商家预写专属 KB,届时 Mem0 是商家画像的唯一来源,价值远大于这里测到的边际**。6.3 诚实界定了「已有专属 KB 时」Memory 的边际下限,真实场景只会更高。

**可讲性**:✅ 强。诚实驱动(主动暴露 RAG 冗余 + CC 自己「可能 nil」被数据否决,见 `methodology_log.md` 方法论 12 实例 10),却落在 Memory 不可替代的硬数字上。**关联**:`ablation_6_3_report.md §1` + 方法论 12 实例 10 + 方法论 1。

---

## 候选故事 9:LLM-as-judge 的 scope 边界 — 裸 LLM 编造数字骗过 calibrated judge(6.3 主线 2)

**6.3 实测**(`ablation_6_3_report.md §2`):主线 2(系统 vs 裸 LLM)judge 自动评分 full 26.7% vs 裸 6.7%,χ²=3.6 **inconclusive**(精确命中预注册欠功效分支)。

**翻转点**:人工核查发现裸 LLM 的 2 个「通过」全是 **judge 假阳**——q_028 凭空编造各价位退款率(18.5%/12.3%/…)、q_068 假设式「设总 GMV 为 X…Y/30>X/12」,judge 看数字「像模像样」给了 pass。**剔除后裸 LLM 真实正确率 = 0%**(无数据接入本就该 0%),full 26.7% vs 0%,χ²=8.0 显著。

**根因 = judge scope 边界**:6.2 校准 judge(α=0.856)用的是**真实系统输出**,**从未校准过「裸 LLM 凭空编造数字」这种分布** → 对裸 LLM 假阳。**与 6.2 strategy judge 能力边界(Spearman 0.605)前后呼应**:两处都是「judge 只在校准过的分布内可信」的同一洞察,6.2 是 strategy 细粒度质量、6.3 是裸 LLM 编造模式。

**可讲性**:✅ 强。展示**始终在严谨识别 judge 的可信边界,而非盲信自动评分**——「judge 给的 inconclusive 我没照单全收,核查发现是 judge 错了」。0% 比任何数字有说服力。**与 6.2 calibration 的 judge 边界叙事构成 stage-6 的「judge 可信范围」主线**。**关联**:`ablation_6_3_report.md §2.2` + `calibration_sampling.md §9`(6.2 judge 边界)。

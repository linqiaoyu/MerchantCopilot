...
11. 声明「高层框架指定改哪个组件」≠ 该组件的实际依赖 blast radius —— 6.4 方案设计,PM 凭印象把"组件 3 = 改 Router `_parse_time_window` 支持多段时间窗"写进框架。CC 调研发现 Router `_parse_time_window` 是**所有 intent(data_query/attribution/strategy)的公共入口**(经 `state["time_window"]` 消费)→ 改它的输出结构会波及 attribution/strategy 的 time_window 读取,blast radius 远大于"只服务 cross_period 多段"的需求。CC 纠正:多段解析放 **metric_query 节点** `_parse_periods`(隔离在 metric 路径),工具层和 Router 层都不碰多段。**教训:高层框架指定的「改哪个组件」是声明,该组件的实际依赖范围(谁消费它)是执行层事实;为满足特定路径需求去改一个公共入口 = blast radius 过大,应核实组件依赖再选最小隔离面**。叠加方法论 7(先调研主代码再列方案)。PM 认领框架定错,采纳节点隔离。
12. 声明「某 tag 已在远程」≠ 它真在远程 —— push 收尾(2026-06,GitHub push 后核实),PM 在 push 流程 step 5 写「确认 stage-6 + eval-dataset-v1.1 都在远程」,**假设 eval-dataset-v1.1 已在远程**。CC `git ls-remote --tags origin` 实证:远程**只有 stage-6**,eval-dataset-v1.1 是本地独有(6.1 打的 tag 指向 a60511b,而 6.1-6.4 直到本次才 push,故 6.1 的 tag 从未上远程)。CC 报给 PM + **没擅自补推**(授权只含 `main` + `stage-6`,多推 tag 是额外不可逆动作)。PM 认领是自己 prompt 的错误假设,补授权推 eval-dataset-v1.1。**教训:涉及远程状态的「确认 X 在远程」措辞,先 `ls-remote` 实证再行动,不因 prompt 说「确认在」就假装在**。**★ 双向性强化**:这次 gap 在 **PM 的 prompt 假设**里(不是 CC 残留假设)—— 与实例 4/7/8(CC 自己的 gap)、实例 5/6(PM 身份/授权声明)、实例 9-11(PM 框架定义 + CC 自摆乌龙)合起来,证明「声明≠执行」**不分谁说的**:CC 出、PM 出都高频,默认核实习惯对双方都适用。

**★ 元观察**:方法论 12 在沉淀后连续照出**十二个**同构实例(实例 1-4 在 6.1 收尾同一轮、实例 5-8 在 6.2、实例 9-10 在 6.3、实例 11 在 6.4 方案设计、实例 12 在 push 收尾;其中实例 4 是 CC 用刚沉淀的方法论照出自己的旧假设残留,实例 5/6 是 CC 照出 PM 的身份声明错 / 授权未核对,实例 7/8 又是 CC 自己没核物理值/输入完整性,实例 9 是 CC 调研 import 照出 PM 的配置开关定义错 + 同轮 CC 自己的读取路径乌龙,实例 10 是 CC + 云端都漏核 RAG KB 冗余携带画像),显示「声明层 ≠ 执行层」是人机协作中高频反复出现的 gap 模式,**值得作为默认核实习惯**(涉及完整性/可追溯性的节点,先核实物理层事实再行动)。这条方法论的自我繁殖能力本身是其有效性的证据。**子模式留痕(四级递进)**:① 实例 4(A/M 凭直觉判)是「grep 到名 ≠ 核到值」—— 核值存在;② 实例 7(key 名在但值空)、实例 8(评了 30 条但 4 条 chunks 空占位)递进为「形式齐了但内容空/残缺占位」—— 核值**完整有效**,不只核值存在;③ 实例 9 递进为「配置开关存在且有效 ≠ 它对目标子集有效果」—— 核**开关与目标节点的依赖链路**(开关一个目标节点不消费的能力,Δ 恒 0);④ 实例 10 再递进为「目标节点消费它 ≠ 它是该信号的唯一来源」—— 核**数据源冗余**(其他源冗余携带同一目标信号,Δ 被稀释)。配置/状态/输入/消融核实必须核到内容完整 + 依赖可达 + 数据源不冗余(`git ls-files` 核跟踪态、`wc -c` 核值长度非空、逐条核 judge 输入字段齐全、grep node imports 核开关-目标依赖链路、grep KB/数据源核目标信号是否被冗余携带)。
...
## 6.4 沉淀(责任分层实战 + 红线披露 + 拒绝 teaching-to-test)

### 方法论 8 责任分层实战(数据层 vs 呈现层,node_data 逐条核实定位归属)

**内容**:修好一个组件会暴露下一个下游瓶颈;责任归属要靠**逐条核实中间产物**(不是凭最终 pass/fail 猜)。
**实战来源**:6.4 task #26 修复后,bad case 通过率 0%→70%,剩余 3 条 fail(q_025/068/069)。**逐条核 node_data 实证**:4 源 uv+orders 全在 / 日均+days=真值 / 3 月×3 指标全在 → **数据层 100% 修对,瓶颈下移到 Insight 呈现层**(LLM 编辑成叙事丢结构化值)。归属精确到「metric_query 数据层 ✅ / Insight 呈现层 ❌」,不模糊说「还有 3 条没修好」。**教训:bad case 残留 fail 必须核中间产物(node_data)定位是哪一层,数据层对 ≠ 端到端对,中间隔着呈现层**。沉淀阶段:6.4。

### q_068 红线披露 + 拒绝 teaching-to-test(方法论 12 双向 + 13 + 诚信红线)

**事实**:6.4 修 q_066 的 period headline 改动,**副作用让 q_068 从 run-1 实 pass [1,1,1] → run-3 边界 fail [1,0,0]**,触 PM「原 pass 不退化」红线。

**CC 的处理(三条诚信判断)**:
1. **主动披露**(方法论 12 双向性):红线被自己的修复触发,主动报 mode 翻转,不藏。
2. **诚实归因不甩锅**:核实发现 run-3 答案**客观变好**(gmv→日均归一让 factual_accuracy 稳定=1,run-2 曾因讲原始 GMV 判 0 已修);残留 fail 维度是 grounding(judge 要显式 partial-month 说明,Insight 没 surface)。**run-1 的 [1,1,1] 是 judge 在 grounding 维度的运气**(run-1 答案同样没说 partial-month)→ 不是答案质量真退化,是 judge 固有方差(方法论 13)+ Insight 呈现瓶颈。
3. **拒绝 tune**:可以改 label 喂 grounding 维度让 q_068 转 pass(→ 80%),但**拒绝** —— 改 label 满足 judge 特定维度 = teaching-to-test,且越过 PM 已定的 Insight v2.0 边界。**「7/10 诚实 > 8/10 注水」**。

**教训**:① 红线被自己触发时,主动披露 + 核实中间产物诚实归因(是真退化还是 judge 方差),不甩锅不掩盖;② judge 单次 pass 可能是方差运气,不能当「质量达标」铁证(方法论 13 延伸);③ 能 tune 到好看数字 ≠ 应该 tune,teaching-to-test 和越界都是诚信红线。**这是简历金矿:诚信红线在真触发时怎么处理**。沉淀阶段:6.4。

...
def visible_events(case: dict) -> list[dict]:
    """Apply frozen temporal/feedback semantics without reading expected labels."""
...
def materialize_case(conn: psycopg.Connection, case: dict, embedder) -> dict[str, str]:
    ...
    for event, vector in zip(rows, vectors, strict=True):
        ...
        conn.execute(
            ...
            (event_uuid, run_id, case["merchant_id"], event["kind"], event["subject"], event["predicate"], event["value"],
        ...
        conn.execute(
            ...
            (memory_uuid, event_uuid, case["merchant_id"], memory_kind(case, event), event["subject"], event["predicate"],
             event["value"], content, "[" + ",".join(map(str, vector)) + "]", status, event["occurred_at"], valid_to),
        )
    return mapping


def run_case(conn: psycopg.Connection, case: dict, embedder) -> dict:
    mapping = materialize_case(conn, case, embedder)

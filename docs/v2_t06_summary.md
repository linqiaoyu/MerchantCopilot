...
- 53 条 Policy Gate 场景通过；本地真实 pgvector 集成测试还验证了 20 并发 run 幂等、10 个并发重复 event 投递去重、10 个同语义 active fact 并发写入后恰好保留一个当前事实，以及 1024 维索引写入。`003_memory_active_fact_invariant.sql` 以部分唯一索引兜底，写入路径使用事务 advisory lock 串行化同一语义键。

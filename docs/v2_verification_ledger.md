...
| T13 | 60×6 消融完整性、配对效应/检验、成本延迟/失败样本汇总 | 已验证 | `tests/test_v2_ablation_analysis.py`；`evals/run_v2_memory_ablation.py` 实际运行本地 pgvector 60×6 canonical retrieval matrix，`analyze_v2_ablation.py` 验收完整输入：full 60/60，minus_memory/bare 0/60，raw_history 45/60，no_temporal_policy 40/60；原始/汇总见 `evals/runs/v2_memory_ablation_local_20260812*.json` | 无 |
| T13 | Qwen 校准、完整 Agent/Judge 原始运行、指标门槛与 bad-case 报告 | 未实现 | 2026-08-12 已以现有凭据真实连通 `deepseek-v4-flash`（non-thinking JSON schema smoke，52 tokens）与 `qwen3.7-plus-2026-05-26`（JSON judge-format smoke，410 tokens）；尚无 v2 逐 case Agent/Judge 原始矩阵。Python 3.12 请求需将 `SSL_CERT_FILE` 指向现有 certifi CA bundle，未降低 TLS 校验 | 真人成对校准标注与完整 Agent/Judge 评测运行 |

...
| T11 | Android debug/release APK、Keystore token、两 endpoint smoke、APK 密钥扫描 | 已实现未验证 | native Android 工程已生成；Android Keystore AES-GCM + MethodChannel 只持久化 token 密文，源码契约测试通过；APK scanner 已有 clean/reject 测试。2026-08-12 `flutter build apk --debug` 在 Gradle 配置阶段失败：目标 NDK `28.2.13676358` 未安装；SDK 仍无法识别 command-line tools，许可证状态 unknown | 安装新版 command-line tools 与 NDK 28.2.13676358 并接受许可证；构建/扫描 debug/release APK、Keystore 真机与 local/Cloud endpoint smoke |

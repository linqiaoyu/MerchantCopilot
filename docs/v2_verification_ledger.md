...
| T13 | Qwen 校准、完整 60×6 原始运行、指标门槛与 bad-case 报告 | 未实现 | 2026-08-12 已以现有凭据真实连通 `deepseek-v4-flash`（non-thinking JSON schema smoke，52 tokens）与 `qwen3.7-plus-2026-05-26`（JSON judge-format smoke，410 tokens）；尚无 v2 逐 case Agent/Judge 原始矩阵。Python 3.12 请求需将 `SSL_CERT_FILE` 指向现有 certifi CA bundle，未降低 TLS 校验 | 60×6 原始执行 runner、真人成对校准标注与完整评测运行 |

...
| T13 | Qwen 校准、完整 Agent/Judge 原始运行、指标门槛与 bad-case 报告 | 未实现 | 2026-08-12 已以现有凭据真实连通 `deepseek-v4-flash`（non-thinking JSON schema smoke，52 tokens）与 `qwen3.7-plus-2026-05-26`（JSON judge-format smoke，410 tokens）。DeepSeek no-Memory 历史 80 条 baseline 已真实重跑且严格验证 80/80、0 errors、192,514 tokens、p50/p95 22,231.051/42,603.81ms，见 `v2_deepseek_baseline_80_20260812*.json`；尚无 v2 逐 case full/Memory/RAG Agent/Judge 矩阵。Python 3.12 请求需将 `SSL_CERT_FILE` 指向现有 certifi CA bundle，未降低 TLS 校验 | 真人成对校准标注与完整 Agent/Judge 评测运行 |

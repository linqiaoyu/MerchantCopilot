...
| T13 | Qwen 校准、完整 Agent/Judge 原始运行、指标门槛与 bad-case 报告 | 已实现未验证 | 固定 Qwen 对历史独立 PM 标注 30 条语料真实执行 92 次：binary 18/18 α=1.000（eligible）；strategy 11/12 可解析 Spearman ρ=0.117，q_011 五次无唯一众数，故 strategy reference-only。原始/统计为 `v2_qwen_recalibration_legacy30_20260812*.json`；主体 90 次 provider usage=239,890 tokens，2 次 tie re-sample 未返回 usage。DeepSeek no-Memory 历史 80 条已严格验证 80/80、0 errors、192,514 tokens、p50/p95 22,231.051/42,603.81ms；尚无 v2 逐 case full/Memory/RAG Agent/Judge 矩阵。Python 3.12 请求需将 `SSL_CERT_FILE` 指向现有 certifi CA bundle，未降低 TLS 校验 | 新增独立真人标注以重校准 strategy；完整 v2 Agent/Judge 评测运行与 bad-case 报告 |

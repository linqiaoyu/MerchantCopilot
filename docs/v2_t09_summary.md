...
已验证：固定的 9 个路由已定义；业务请求使用 Bearer demo token；所有改变状态的 POST 请求要求 UUID 格式的 `Idempotency-Key`；SSE 公开词表严格为 11 种事件，成功序列为 `meta → node_started → node_completed → evidence → final → done`，超时失败为 `meta → error → done`。`DemoRuntime` 以 LangGraph `MemorySaver` 维持 thread checkpoint；同 key 的 8 并发请求经契约测试只执行一次 Agent。`auth`、`quota`、`llm_timeout`、`database_unavailable`、`agent_failure` 五类稳定错误码均有本地契约测试。

...
此链路不依赖个人 Cloud Run 或 Supabase 项目；使用者提供自己的 DeepSeek API key。`.env.example` 已给出本地 `55432` DSN，API 会在启动时加载 `.env`，并在 DSN 存在时使用 Postgres runtime 与官方 PostgresSaver。

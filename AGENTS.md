...
这是一个用于面试与简历展示的直播电商商家分析 Agent,不以生产可用性为目标。每次决策都按以下固定优先级判断,上一条永远压倒下一条:

1. **简历对齐** — 简历里每句话都要在代码里有对应实现;不允许"听起来高级但代码里没做"
2. **能跑通** — 演示时不能卡、不能崩;单次响应不超过 5 秒是早期性能目标,当前 strategy 端到端稳态约 17.5 秒,已按“演示项目宁稳勿快”接受,完整取舍见下文“阶段 4b 4 条诚信留痕”第 3 条
3. **能讲清** — 任何技术决策都要能在 2-3 分钟讲明白"为什么这么做"
4. **代码可读** — 面试官可能要求看代码,变量名/注释要专业

⚠️ **任何"上生产/规模化/多租户/高并发"的扩展都不在范围内**,见下方"❌ 不做"列表。

项目使用受控合成数据,而不接入真实商家数据,是两层方法论取舍:

1. **真实平台数据不适合作为面试 demo 的前提依赖。**接入抖音电商开放平台或快手开放平台需要企业或个体工商户资质,并已有实际经营的店铺;淘宝开放平台允许个人注册,但官方未开放直播维度数据。把这些资质与经营条件设为项目运行前提,不符合面试 demo 的边界。
2. **归因评测需要已知 ground truth。**要验证 Agent 能否正确识别“GMV 下降”背后的真实原因,评测者必须事先知道真实原因。受控合成数据可以人为植入已知业务异常,为归因能力提供可核验的 ground truth;这是异常检测类系统的标准评测做法。真实数据若没有独立验证渠道,反而无法支持同等精度的归因测试。

因此,合成数据不是缺少真实数据后的将就方案,而是为可验证的归因实验主动选择的测试基座。

---

## 📍 当前阶段

**阶段 5 ✅ → 阶段 6 评测闭环 ✅ 完成(stage-6 release)。6.1 eval dataset ✅ + 6.2 judge calibration ✅ + 6.3 消融 ✅(主线1 Memory 独家聚合事实 0%→40% χ²=20;主线2 系统 vs 裸 LLM 纠正 judge 假阳后 26.7%→0% χ²=8;见 `evals/runs/ablation_6_3_report.md`)+ 6.4 bad case 修复回归 ✅(task #26 metric_query 解析升级 group_by+多段时间窗,3 组件 additive,通过率 0%→70% χ²=7.0 校正 5.14 显著,回归 16/16;剩余 3 条诚实归因 Insight 呈现层 v2.0;q_068 红线披露+拒绝 teaching-to-test;见 `evals/runs/stage6_4_results.md`)。两组 X%→Y% 入库,消融+bad case 回流构成完整闭环。→ 阶段 7(可选)HITL+流式 待定**
...
3. **端到端延迟 ~17.5s 接受不优化**:4a 7.5s + Mem0/LLM 改写 ~10s,仍超 AGENTS.md
...
6. 评测闭环 ✅ 完成:**6.1 eval dataset ✅ + 6.2 judge calibration ✅ + 6.3 消融 ✅ + 6.4 bad case 修复回归 ✅**(v1.1 80 条 tag `eval-dataset-v1.1`;judge=Qwen-Max 跨家 binary α=0.856 / strategy Spearman 0.605 诚实降级,见 `evals/runs/calibration_sampling.md §9`;6.3 三配置消融 full/-Mem0/裸LLM 主线1 Memory 锚定三度量 + 主线2 系统 vs 裸 LLM,见 `evals/runs/ablation_6_3_report.md`;6.4 task #26 metric_query 解析升级 3 组件 additive 通过率 0%→70% χ²=7.0 校正 5.14 显著 回归 16/16,剩余 3 条归因 Insight 呈现层 v2.0,见 `evals/runs/stage6_4_results.md`)→ `stage-6` tag = 评测闭环 release
7. (可选)HITL + 流式输出
...
| 向量库 | Chroma | 本地起,零运维 |
| 测试与评测 | pytest 9.0.3 + 自建 eval pipeline | 契约测试 + judge 校准、消融与 bad case 回归 |
...
详细架构与设计决策见本文件“Agent 工作流”以及 `docs/stage2_summary.md`、`docs/stage3_summary.md`。
...
## ❌ 不做(看到我或你自己要做以下任何一项,直接停下来提醒我回看 AGENTS.md)
...
- 架构与设计决策:本文件“Agent 工作流”及 `docs/stage2_summary.md`、`docs/stage3_summary.md`
